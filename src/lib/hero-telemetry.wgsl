...
fn computerMark(p: vec2f) -> f32 {
  let q = rotate(p, -0.08);
  let bezel = sdRoundedBox(q - vec2f(0.0, -0.03), vec2f(0.20, 0.14), 0.025);
  let screen = sdRoundedBox(q - vec2f(0.0, -0.03), vec2f(0.175, 0.112), 0.015);
  let stand = sdRoundedBox(q - vec2f(0.0, 0.135), vec2f(0.035, 0.045), 0.008);
  let base = sdRoundedBox(q - vec2f(0.0, 0.18), vec2f(0.095, 0.012), 0.006);
  var mark = opSmoothSub(bezel, screen, 0.008);
  mark = opSmoothUnion(mark, stand, 0.012);
  mark = opSmoothUnion(mark, base, 0.01);
  return mark;
}
...
fn fs_main(@location(0) uv: vec2f) -> @location(0) vec4f {
  ...
  let dComputer = computerMark(computerP);
  ...
  let teal = vec3f(0.0, 0.498039, 0.447059);
  let volt = vec3f(0.909804, 1.0, 0.443137);
  let leftClear = smoothstep(0.34, 0.62, uv.x);
  var a = fill * 0.08 + line * 0.28 + glow * 0.08 + traces * 0.32 + spark * 0.06;
  a *= 0.55 * leftClear;
  a = clamp(a, 0.0, 0.34);
  let col = mix(teal, volt, spark * 0.45 + line * 0.05);
